...
// POST /api/exams/sync-subject-tests — one-time cleanup: removes leftover
// exams from an earlier version of this feature that materialized chunked
// "model test" exams per subject. বিষয়ভিত্তিক now reads straight from the
// question bank as a paginated reading list (see /api/questions/public/
// reading-list), so any is_auto_subject rows still around are just orphaned
// clutter — nothing in the UI links to them anymore. Safe to call anytime.
router.post('/sync-subject-tests', requireAdmin, asyncHandler(async (req, res) => {
  const { rowCount } = await pool.query('DELETE FROM exams WHERE is_auto_subject = true');
